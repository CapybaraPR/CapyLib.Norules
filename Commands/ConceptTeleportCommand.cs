...
public sealed class ConceptTeleportCommand : ICommand
{
    ...
    public string[] Aliases => new[] { "concepts_tp", "cpt" };
    public string Description => "Телепорт к структурам концептов (для отладки)";
    ...
    public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
    {
        ...
        {
            response = "\n<color=#38bdf8><b>[ Concept Teleport ]</b></color>\n" +
                       "<color=#a3e635>• cptp co2</color> — Панели СО₂ (HczHid)\n" +
                       "<color=#a3e635>• cptp hackers</color> — Панель Хакеров (HczServerRoom)\n" +
                       "<color=#a3e635>• cptp 008_173</color> — Трубка 008 (Lcz173)\n" +
        ...
        if (destination == null)
        {
            response = $"<color=red>Неизвестная точка '{target}'.</color> Введите <b>cptp list</b> для списка.";
            return false;
        }
