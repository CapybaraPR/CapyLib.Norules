...
namespace Capy.NoRules.Concepts.Co2;
...
public sealed class Co2Concept
{
    ...
    private void OnRestartingRound()
    {
        ...
        try { AudioToggle.DestroyGlobal("co2"); } catch { }
        try { AudioToggle.DestroyGlobal("co2group"); } catch { }
    ...
    private void BuildPanel(Vector3 roomPos, Quaternion roomRot, bool left)
    {
        ...
        var rr = roomRot * Quaternion.Euler(0f, rotY, 0f);
    ...
    /// <summary>
    /// Проверка и спавн группы СО2 из кастомного спавнера волн.
    /// </summary>
    public bool CheckSpawnGroup(List<Player> spectators)
    {
        if (!_config.IsEnabled || _spawnedThisRound || !CanAutoSpawn())
            return false;

        if (spectators == null || spectators.Count < Math.Max(1, _config.MinSpectators))
            return false;

        var squad = spectators.Take(_config.SquadSizeMax).ToList();
        RegisterSquad(squad);
        return true;
    }

    /// <summary>
    /// Добавляет одного игрока в отряд СО2 с указанным рангом (для .gcr).
    /// </summary>
    public void AddMember(Player player) => AddSpecialist(player);

    public void AddCaptain(Player player)
    {
        if (player == null) return;
        if (!player.IsAlive)
            player.Role.Set(RoleTypeId.NtfCaptain, SpawnReason.None);

        _squadMembers.Add(player.UserId);
        CustomUnits.AssignMember(player, "Отряд СО2", "#14b1e0", "Капитан");
        ApplyKit(player, 1);
        _spawnProtection[player.UserId] = DateTime.UtcNow.AddSeconds(10f);

        player.ShowZoneHint(HintZone.TopCenter,
            "<color=#14b1e0><b>☠ Вы — Капитан ОТРЯДА-СМЕРТНИКА СО₂</b></color>\n" +
            "<size=65%><color=#c2c2c2>МОГ • Активируйте обе панели в комнате HID в HCZ ([E])</color></size>",
            10f, "co2_brief", 20);
    }

    public void AddSpecialist(Player player)
    {
        if (player == null) return;
        if (!player.IsAlive)
            player.Role.Set(RoleTypeId.NtfSpecialist, SpawnReason.None);

        _squadMembers.Add(player.UserId);
        CustomUnits.AssignMember(player, "Отряд СО2", "#14b1e0", "Специалист");
        ApplyKit(player, 2);
        _spawnProtection[player.UserId] = DateTime.UtcNow.AddSeconds(10f);

        player.ShowZoneHint(HintZone.TopCenter,
            "<color=#14b1e0><b>Вы — Специалист отряда активации СО₂ МОГ</b></color>\n" +
            "<size=70%><color=#c2c2c2>[E] рядом с панелями в HID для активации CO2</color></size>",
            8f, "co2_brief", 20);
    }

    public void AddCadet(Player player)
    {
        if (player == null) return;
        if (!player.IsAlive)
            player.Role.Set(RoleTypeId.NtfPrivate, SpawnReason.None);

        _squadMembers.Add(player.UserId);
        CustomUnits.AssignMember(player, "Отряд СО2", "#14b1e0", "Кадет");
        ApplyKit(player, 8);
        _spawnProtection[player.UserId] = DateTime.UtcNow.AddSeconds(10f);

        player.ShowZoneHint(HintZone.TopCenter,
            "<color=#14b1e0><b>Вы — Кадет отряда СО₂ МОГ</b></color>\n" +
            "<size=70%><color=#c2c2c2>[E] рядом с панелями в HID для активации CO2</color></size>",
            8f, "co2_brief", 20);
    }
    ...
    private void RegisterSquad(List<Player> squad)
    {
        ...
        try { AudioToggle.CreateGlobal("co2group", volume: 1.0f); } catch { }
    ...
    private void Activate(List<string> activators)
    {
        ...
        try { AudioToggle.DestroyGlobal("co2group"); } catch { }
        try { AudioToggle.CreateGlobal("co2", volume: 1.0f); } catch { }
    ...
    {
        // 1. Окно отмены (90 секунд от старта): лифты работают, игроки могут перемещаться и отменять протокол
        ...
        _allowCancel = false;
        BroadcastToAll("<color=#ef4444><b>[ ВНИМАНИЕ ]</b></color> <color=#cbd5e1>Точка невозврата! Отмена протокола CO2 заблокирована.</color>");

        // 2. Пауза до кульминации 1:51 (21 секунда, 90 + 21 = 111 секунд)
        yield return Timing.WaitForSeconds(Mathf.Max(5f, _config.PoisonGraceSeconds));

        if (!_activated)
            yield break;

        // 3. ФИНАЛ (1:51): Лифты на Поверхность запечатываются (как при детонации БГ) и начинается пуск газа
        LockElevators(force: true);
        BroadcastToAll("<color=#0099ff><b>[ ВЫБРОС ГАЗА CO2 ]</b></color>\n<size=70%><color=#cbd5e1>Комплекс загерметизирован. Вентиляция заполнена смертельным газом!</color></size>");
        ...
        BroadcastToAll("<color=#0099ff><b>[ ПРОТОКОЛ CO2 ВЫПОЛНЕН ]</b></color>\n<size=70%><color=#cbd5e1>Внутренний комплекс изолирован и зачищен от биологического персонала.</color></size>");
    }
    ...
    private IEnumerator<float> PoisonLoop()
    {
        while (_activated)
        {
            foreach (var pl in Player.List)
            {
                try
                {
                    // Газ действует только внутри комплекса (на открытом воздухе Surface игроки не травятся)
