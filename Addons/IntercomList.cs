    ...
    private IEnumerator<float> IntercomLoop()
    {
        while (Round.IsStarted)
        {
            try
            {
                ...
                {
                    sb.AppendLine($"<size=64><color=#ff3333><b>ГОВОРИТ:</b> {Intercom.Speaker.Nickname}</color> ({(int)Intercom.SpeechRemainingTime}с)</size>");
                }
                else if (Intercom.RemainingCooldown > 0f)
                {
                    sb.AppendLine($"<size=60><color=#ffaa33>Перезарядка: {(int)Intercom.RemainingCooldown}с</color></size>");
                }
                else
                {
                    sb.AppendLine("<size=64><color=#a3e635><b>ИНТЕРКОМ ГОТОВ</b></color></size>");
                }

                sb.AppendLine("<size=40><color=#ffa94e>══════════════════════════</color></size>");
                ...
                sb.AppendLine($"<size=44><color=#ff9933>Класс D:</color> <b>{classD}</b>  |  <color=#ffff33>Учёные:</color> <b>{scientists}</b></size>");
                sb.AppendLine($"<size=44><color=#999999>Охрана:</color> <b>{facilityGuards}</b>  |  <color=#3399ff>МОГ:</color> <b>{mtf}</b></size>");
                sb.AppendLine($"<size=44><color=#33cc33>Хаос:</color> <b>{chaos}</b>  |  <color=#ff3333>SCP:</color> <b>{scps}</b></size>");
                sb.AppendLine($"<size=36><color=#aaaaaa>Наблюдатели: {spectators}</color></size>");
                sb.AppendLine("<size=40><color=#ffa94e>══════════════════════════</color></size>");
